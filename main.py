...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "https://ehmt8mro7sonvp9cs5oblz.streamlit.app",  # Your Streamlit Cloud URL
        "https://hg-resume-craft-backend.vercel.app"  # Replace with your Vercel URL
    ],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
...
class ResumeInput(BaseModel):
    ...
    @field_validator('education', 'skills')
    @classmethod
    def validate_non_empty_lists(cls, v):
        if not v or all(not item.strip() for item in v):
            raise ValueError("List cannot be empty or contain only empty strings")
...
# Chatbot endpoints commented out due to missing chatbot_service module
# @app.post("/api/chatbot")
# async def chatbot_endpoint(request: dict):
#     """Enhanced AI-powered chatbot with web search and smart capabilities"""
#     try:
...
#             raise HTTPException(status_code=400, detail="Message is required")
#         
#         return {"message": "Chatbot endpoint not implemented due to missing chatbot_service"}
#         
#     except HTTPException:
#         raise
#     except Exception as e:
...
#     try:
#         session_id = request.get("session_id", "default")
#         return {"summary": "Session summary not implemented due to missing chatbot_service"}
#         
#     except Exception as e:
#         logger.error(f"Error getting session summary: {str(e)}")
#         raise HTTPException(
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.getenv("PORT", 8000))  # Use PORT env variable for Vercel/Heroku
